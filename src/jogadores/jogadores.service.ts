...
export class JogadoresService {
  constructor(@Inject('JOGADORES_REPOSITORY') private readonly jogadoresRepository: Repository<Jogadores>) {}

  private readonly logger = new Logger(JogadoresService.name);

  async criarJogador(criarJogadorDto: CriarJogadorDto): Promise<Resposta> {

    const isjogadorAlreadyCreated = await this.jogadoresRepository.findOneBy({ email: criarJogadorDto.email })
  ...
  async atualizarJogador(id: number, atualizarJogadorDto: AtualizarJogadorDto): Promise<Resposta> {
    const jogador = await this.jogadoresRepository.findOneBy({ id });

    if (!jogador) {
      throw new NotFoundException({
        statusCode: 404,
        message: `Jogador com o id ${id} não encontrado`,
        data: {},
      });
    }

    const { nome, telefoneCelular } = atualizarJogadorDto;
    const jogadorUpdated = await this.jogadoresRepository.update(id, { nome, telefoneCelular });
    
    if(jogadorUpdated.affected === 0) throw new BadRequestException({
      statusCode: 400,
  ...
  async listarJogadores(): Promise<Resposta>{
    ...
    if(allJogadores.length <= 0) throw new BadRequestException({
      statusCode: 400,
      message: `NÃ£o existem jogadores cadastrados`,
      data: {}
    })
  ...
  async apagarJogador(id: number): Promise<Resposta> {
    ...
    if (!jogador) {
      throw new NotFoundException({
        statusCode: 404,
        message: `Jogador com o id ${id} nÃ£o encontrado`,
        data: {},
      });
    }

    const isJogadorDeleted = await this.jogadoresRepository.delete(id);
    if(isJogadorDeleted.affected === 0) throw new BadRequestException({
      statusCode: 400,
      message: `NÃ£o foi possÃ­vel apagar o jogador com o id ${id}`,
      data: {},
    })
  ...
  private async criar(criarJogadorDto: CriarJogadorDto) {
    ...
    const jogadorCreated = await this.jogadoresRepository.save({ nome, telefoneCelular, email });
    return jogadorCreated;
  }

  async consultarJogadorPeloEmail(email: string): Promise<Resposta> {
    const jogadorEncontrado = await this.jogadoresRepository.findOneBy({ email });
